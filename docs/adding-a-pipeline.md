...
Create a new subdirectory `src/forecast/my_new_kind/` and put `my_new_kind_pipeline.h`/`.cpp` there, along with any helper classes private to this kind (rollout strategy, fuser, uncertainty propagation, etc. — whatever is analogous to `stacked_ensemble/`'s `ensemble_fuser.h`, `unscented_transform.h`). Only `src/forecast/` itself holds kind-agnostic plumbing (`pipeline.cpp`, `pipeline_registry.*`, `config_builder.cpp`); `src/forecast/backends/` holds cross-kind, pluggable implementation-selection abstractions — the ONNX/LibTorch `IModel` abstraction shared by any kind that runs neural-net inference, `IICSource` (IC-kind dispatch via `make_ic_source()`, `ic_source_factory.h`), and `IDecoder` (decoder-kind dispatch via `make_decoder()`, `decoder_factory.h`) — reuse these rather than duplicating a model-backend, IC-loading, or latent-to-grid-decoding wrapper inside your new kind's subdirectory. A kind with no learned models at all (e.g. a closed-form matrix/ODE rollout) may not need the `IModel` half of `backends/` at all, but will still likely need `IICSource`/`IDecoder` if it seeds a latent state and decodes it to a physical grid the same way.
